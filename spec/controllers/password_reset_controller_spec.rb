...
RSpec.describe PasswordResetController, type: :controller do
  ...
  context 'POST #create_password_reset' do
    ...
    it 'creates new password reset digest for existing account' do
      ...
      expect {
        post :create_password_reset, params: { password_reset: { email: 'norbert@example.com' } }
    ...
    it 'sends password request email on password reset request for existing account' do
      ...
      expect {
        post :create_password_reset, params: { password_reset: { email: 'birte@example.com' } } 
      }.to change { UserMailer.deliveries.count }.by(1)
    end

    it 'does nothing if no account was found to reset password for' do
      post :create_password_reset, params: { password_reset: { email: 'unknown@nowhere.com' } }

      expect(response).to redirect_to :request_password_reset
    end
  end
  ...
  context 'GET #reset_password' do
    ...
    it 'allows password reset for valid token' do
      ...
      get :reset_password, params: { id: user.id, token: user.password_reset_token }
    ...
    it 'alerts and redirect to root url if link is invalid' do
      ...
      get :reset_password, params: { id: user.id, token: 'Some invalid token' }
  ...
  context 'PATCH #set_new_password' do
    ...
    context 'passwords match' do
      it 'sets new passwords if inputs match' do
        ...
        patch :set_new_password, params: {
          id: @user.id, token: @user.password_reset_token,
          new_password: {
    ...
    context 'new passwords do not match' do
      it 'does not change password' do
        ...
        patch :set_new_password, params: {
          id: @user.id, token: @user.password_reset_token,
          new_password: {
            password: 'Some passwd', password_confirmation: 'Some other passwd'
          } 
        }
    ...
    context 'endpoint protection' do
      it 'cannot access endpoint if not authenticated' do
        ...
        patch :set_new_password, params: { new_password: {
          password: 'Some passwd', password_confirmation: 'Some passwd' }
        }
